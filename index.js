/*
* JANGAN REEDIT SEMBARANG NANTI BISA² ERROR
* DI YOUTUBE ARDIGANS YT SUSAH SAYA KASI TUTORIALNYA
*NANTI KALO ERROR NYALAHIN GUA🤬 INGAT KASI AUTHOR
*ARDIGANS
*WA 085282996146
* TANYA DISINI☝️ KALO ADA YANG ERROR
*/
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:3.0\n' 
            + 'FN:ARDIYANSAH\n' // jangan diganti!! nanti orror'
            + 'ORG:Owner Gans❤️😆;\n' // gapapa diganti tapi sebaiknya tidak
            + 'TEL;type=CELL;type=VOICE;waid=6285282996146:+62 852-8299-6146\n' // 🚫nanti orror'
            + 'END:VCARD'
/******END OF VCARD INPUT******/

prefix = '!' //seterah kalian
blocked = []
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' silahkan scann QR Nya By PATRICK'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo🐦🐦 @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}* yang betah ya Bowh😆`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ [WAIT] *SABAR BROWH PATRICK SEDANG MEMBUAT*',
				success: 'BERHASILL☑️',
				...
				only: {
					...
					Badmin: '[❗] BOT HARUS JADI ADMIN! ❌',
                                        daftarB: `── 「REGISTER PATRICK」 ──\n*「 BELUM DAFTAR 」*!\nDengan cara daftar, \n\nCommand : ${prefix}daftar nama|umur\nContoh\nketik : ${prefix}daftar PATRICK|20`,
				}
			}
			...
			const ownerNumber = ["6285282996146@s.whatsapp.net","628********@s.whatsapp.net"] // Ganti Nomerlu Yng ****
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '6285282996146@s.whatsapp.net'
                        /******ApiKey Input******/
...
case 'timer':
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               client.sendMessage(from, virtex(prefix, sender), text, {quoted: mek})
                                                    break
               case 'daftarvip':
               client.sendMessage(from, daftarvip(prefix, sender), text, {quoted: mek})
               break
               case 'kodenegara':
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `*PATRICK TELAH MENURUNKAN JABANTANNYA☑️* :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`*PATRICK TELAH MENURUNKAN JABANTANNYA☑️* @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'PATRICK TELAH MENAIKKAN JABATANNYA SEBAGAI ADMIN 😆 :\n'
			for (let _ of mentioned) {
			    teks += `@${_.split('@')[0]}\n`
			}
			mentions(teks, mentioned, true)
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, '*Tuh owner PATRICK😆❤️*',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 TAG SEMUA MEMBER 〙✪══\n╠➥'+teks+'╚═〘 PATRICK 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'ocr':
                    ...
                    reply('pesan report ke Owner PATRICK telah terkirim, pesan hoax/palsu gak di respon.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
                ...
                break
                   ...
                   break
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`ALREADY REGISTRASI PATRICK\`\`\`\n\n\`\`\`PADA ${date} ${time}\`\`\`\n\`\`\`[NAMA]: ${jeneng}\`\`\`\n\`\`\`[NOMOR]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[UMUR]: ${umure}\`\`\`\n\`\`\`Selamat anda udah daftar\`\`\`\n\`\`\`\`\`\`\n\`\`\`ketik ${prefix}help untuk mendapatkan fitur PATRICK\`\`\`\n\`\`\`\nTotal Yang bergabung PATRICK ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
